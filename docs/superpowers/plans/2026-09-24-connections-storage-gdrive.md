...
| 2 storage interface | **done** — `services/storage/local.js` (the Server backend: put/open/remove/urlFor/owns/health, traversal-safe) + `services/storage/index.js` (the registry). `artifactUpload` reads EXIF + thumbnails from the TEMP file, then hands the bytes to the backend; `storeFileFromPath` (connection import — was a hand-copied uploader, audit A4) and `storeBareFile` (`/api/images/upload`) go through it too. Test `storageBackends` (A/B'd: reversing the metadata-before-put order fails it). |
| 3 connections | **done** — `models/Connection` (credentials encrypted ON the record with Secret.js's AES-GCM, deliberately NOT in the Secret collection, which operations can read as `$secrets.X`), `services/connections` (list/default/rename/remove, Server synthesized and protected), `GET/PATCH/DELETE /api/v1/connections`, `PUT /api/v1/me/storage` (session JWT). The registry reads the user's default and falls back to the Server for any type it cannot build or that throws (decision 1). Test `storageConnections` (A/B'd). |
| 4 Google connect | **done (code), not exercised against real Google** — `services/googleOAuth.js`: `POST /api/connections/google/start` (session) returns the consent URL (scope `drive.file` only, `access_type=offline`, `prompt=consent`); the user is named in a signed 15-min `state`, since Google's redirect carries no session header. `GET /api/connections/google/callback` exchanges the code, reads the account email, find-or-creates the "Moduli uploads" folder, stores the refresh token ENCRYPTED on the Connection, and redirects to `/?connections=1&google=…` (the app opens Connections and toasts the outcome). **Reconnecting the same Google account (or a named `reconnectId`) updates that connection in place — its id is in every stored `gdrive:` ref, so a new id would orphan them.** 503 with the missing env names when §5 is not done. Plain REST over `fetch`, no Google SDK dependency. |
| 5 gdrive backend + /files | **done (code), not exercised against real Google** — `services/storage/gdrive.js` (put = resumable upload into `Moduli uploads/<YYYY-MM>` under the file's ORIGINAL name; open with Range; remove; health; a cached access token, one retry on 401; a revoked token sets `needs_reconnect`). A failed put leaves the temp file, and **`registry.putForUser`** stores it on the Server instead with `storageFallback` in the upload result (decision 1 — covers a Drive that fails at PUT time, not just one that cannot be built). `GET /files/:connectionId/:fileId` streams through the server (`services/storage/serveFile.js`: Range → 206, ETag = the ref, `private, max-age=30d, immutable`, 304, 404 "missing in Drive", 502 `needs_reconnect`). `GET /api/connections/:id/health` refreshes the stored status. Client: `resolveFileRef("gdrive:c:f")` → `/files/c/f` (Task 7's one branch — without it a Drive-stored file would not render), and a **Storage** section at the top of the Connections tab (default radio, Connect / Reconnect / Disconnect, health dot) — a first slice of Task 8. Tests: `server/__tests__/storageGdrive.test.js` (13, fake Drive through injected fetch; A/B: removing the put-fallback fails its test, dropping same-account reuse fails the reconnect test), `client/src/__tests__/StorageConnections.test.jsx` (3). **Not done:** the bounded server LRU cache from §2.5a; the /files route does not check a row references the file (parity with public `/uploads`; `drive.file` limits it to app-made files). |
| 6 | mostly already true — `storeUploadedFile`/`storeBareFile` go through `putForUser`; shares inherit it. Dedup across backends still keys on SHA-256. Not separately verified. |
| 7 | **done** with Task 5 (see above). |
| 8–11 | not started. **Before any of it is real: do the §5 Google Cloud setup and set the three env values on the server.** |
