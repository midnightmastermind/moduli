...
### 2026-09-03 (12) — THE CATALOGUE SHIPS PROJECTED, and the rest of a row arrives when something looks at it

(11) split the catalogue phase and the answer was unambiguous:
```
...
nothing**, which kills the "batch the catalogue write" idea outright. And
**nothing happens between content being ready at 2.4s and the first chunk at
7.6s except socket.io receiving, inflating and parsing the frame**, because the
mark fires when our handler runs. That window holds a 2,354ms task and a
1,047ms one. It is the bytes.

**AND (11) ALSO PROVED NOT ALL BYTES ARE EQUAL — a result worth more than the
change that produced it.** Stripping 3.6 MB of nulls moved the measurement by
NOTHING (`ops:start` 9,667 -> 10,197ms against a 4-10s noise band).
`"ownStyle":null` x15,708 compresses to nearly nothing and parses instantly.
**Cheap bytes.** What is expensive is high-entropy content — URLs, titles,
artist names, ISBNs — which is exactly `fields` (3.36 MB) and `meta` (3.45 MB).

**SO THE DEFERRED HALF SHIPS PROJECTED**, and the keep-set is DERIVED from the
grid's own declarations — operation pipelines, `optionsSource` predicates, the
grid's filters — never written down. The user's constraint was explicit ("as
long as nothing is hardcoded toward media"), and a list like `["Board Category",
"Owned", "Tags", "Episodes"]` would be four renames from wrong AND would teach
the wire what a media board is.
```
fields   3.36 MB -> 0.89 MB      meta   3.45 MB -> 1.54 MB
```
**OVER-MATCHING IS SAFE AND UNDER-MATCHING IS A SILENT WRONG ANSWER**, so
referencing is a blunt substring test on purpose: a field kept for nothing costs
bytes, one missed makes an operation compute against an absent value.

**AND THE FIRST DERIVATION WAS INERT — the projection removed NOTHING.**
Serializing the field records whole meant **every field matched its own `id`**,
so `isReferenced` answered true for all 292. Caught by the control asserting
that something is dropped; without it three green equivalence tests would have
"proved" a projection that projected nothing. *Modules are deliberately still
excluded: `fieldBindings` names a field because a module RENDERS it, which is
the render path, and including them marks nearly everything referenced.*

**WHAT MAKES IT SHIPPABLE WITHOUT A FLAG IS THE EQUIVALENCE GATE.** The last two
load changes went out opt-in and were settled on the device; the user asked for
no more URL typing, so the proof moved into the suite:
`deferredProjectionEquivalence.test.js` runs the live grid's own 71 pipelines
over full rows and projected rows and requires **byte-identical effects**, and
requires **every find-mode dropdown to resolve an identical option list** — the
half a sweep-only check would miss, since 60% of the option pool is the
catalogue (2026-09-01 (3)) and an ancestor-scoped dropdown silently resolving to
ZERO is a documented failure here. Both carry controls: at least some dropdowns
must actually resolve non-empty, or "identical" is two empty lists.

**HYDRATION IS THE OTHER HALF, and it fails toward showing something.**
`ModuleInstance` asks for the rest of a row when it renders a partial one;
`helpers/occurrenceHydration.js` batches a commit's worth into one request, and
`renderWindow` virtualises above 120 rows so opening a 5,484-row board asks for
a screenful. **In-flight ids are remembered** — a partial row re-renders while
its own reply is outstanding and would otherwise queue itself again, compounding
into a flood — and they are released on the RESPONSE rather than on a timer,
because a picked timeout is a guess about latency this file has been bitten by
before. The server **always replies, even with nothing**, or a declined id stays
pending forever and can never be asked about again.

**AND `getUserCache` DOES NOT EXIST.** The first version of the handler called
it — the `watchRegion is not defined` class this repo has shipped twice — and
reading what `registerStateHandlers` destructures is what caught it, not the
build. It uses `userCacheReady` + `ensureUserCache`, and **refuses rather than
falling back to a cold read**: a per-render database read on a 21,000-row grid
is the ~180s cost the prewarm exists to avoid.
