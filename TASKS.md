...
| 19 | **A task you ever SCHEDULED disappears from the Tasks page forever.** Dropping a task on a schedule slot stamps `Date` (`Schedule: Stamp Date/Time Slot`), and the Tasks page inherits the grid's `Date = today` filter — so **7 of the 20 rows on Tasks are invisible**, including `Sign up for peer support mentor class` (incomplete, `Due 2026-08-11`) and `Therapy with Keith` (tomorrow). **`dueSpan` is NOT the bug** — it lists an outstanding task to its due date plus a 3-day grace, so schedule placement correctly stopped on Aug 14; nothing brings it back to the task LIST. Measured: the overdue task is listed by Occupational (Tasks, hidden), a Todo on the Aug 17 column (hidden) and one unreachable Todo — **invisible everywhere**. The fix is a product decision about three filter states that already exist and differ (`inherit` = `{}` · `cleared` = `{value:null,unit:"day"}` which means *show nothing dated* · a key ABSENT which passes everything), so it needs your call rather than a guess | ❓ your call, mechanism fully measured |
