...
| 3b1 | **Two trackers are named for a period they do not scope to — right today only by accident** — `Nutrition: Today's Micronutrients` and `Bills: Paid This Month` carry **no date rule at all**: their predicate is ancestor + not-a-feed-copy + `Completed IS true` + a value check. Micronutrients is correct ONLY because the Schedule page currently holds exactly one day column; a second one (a date range, or tomorrow before the sweep) sums both days into "Today's". Bills reads 0 because no bill is ticked yet — vacuously right, and it would sum every completed bill ever the moment one is | 🐛 latent, confirmed |
| 3b2 | **And the two wall-clock trackers ignore the filter by construction** — both `FIND $allContainers WHERE Schedule Format IS "day-col" AND Date SAME_DAY "$today"`. Navigating the Trackers page to another day leaves them showing today. Whether they SHOULD follow the page filter is your call — it is what the audit ask implies, but it changes a tile you read daily | ❓ your call |
