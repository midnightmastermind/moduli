...
| H2 | **The INPUT section sits below a long DISPLAY list** — noticed while verifying H on prod. `selectedFirst` floats ticked fields to the top *within* each section, but DISPLAY is the whole first section, and this grid has ~99 display fields — so the controls you came to type into need a scroll. Your 08-21 ask was *"display and then input fields seperated by section"*, so the order is deliberate; floating ticked fields above BOTH captions, or putting INPUT first, would each fix it | ❓ your call |
