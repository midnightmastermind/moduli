...
| L | **BOOK LIBRARY RECONCILIATION** — *"look at the Calibre Library in my windows Documents folder and the book files in Documents folder of the drive and compare the two libraries while also coming up with one big organized book manifest so i can merge and organize these books better."* (08-24). **PLANNED, not built:** `docs/superpowers/plans/2026-08-24-book-library-reconciliation.md`. **Surveying first changed the premise twice.** It is **FIVE libraries, not two** (1,816 books, ~13GB, seven locations) — and **the C: "Calibre Library" is not one**: no `metadata.db`, 95 books in author folders, 144 dumped flat, 279 MoonReader page-cache PNGs. D: holds the two REAL catalogs (`big_books` 792 · `Calibre Library` 454). **And counting by extension would have put 3,194 WORK DOCUMENTS in the book manifest** — `work_files` is certificates, pay stubs and a 1Password kit; excluded by folder and the exclusion is reported, never silent. User's calls: **report first** (read-only, nothing moved or imported) and **flag duplicates, NEVER auto-delete**. Open: D: needs mounting for hashing (`sudo` wants a password), and **`big_books` is the largest library and the ask did not name it** — confirm scope | 📋 planned, awaiting the two answers |
| M | **CODEX AND BOOKMARKS ARE NOT ON THE GRID** — *"i also didnt see the codex or the bookmarks in the grid"* (08-24). **The data is intact and the route to it is sidebar-only.** Measured: `Root/Bookmarks` → page → board listing **1,467** rows, all parented; `Root/Codex` → 8 subfolders, **75** pages. The three panels carry 25 tabs between them and **neither is among them** — the imports created these in the FILES manifest and never pinned them to a panel. The created-but-not-PLACED distinction: every data check passes and the thing still is not where the user looks. Fix is one migration pinning both as tabs — **user's call, since it changes their panel layout** | ⏳ awaiting the user |
| M | **THE SPOTIFY EXPORT CARRIES NO PLAYLISTS** — *"where did we end up on the music data being too small"* (08-24). **Not truncated, and measured rather than assumed:** the CSV ends on a complete row and all 4,443 rows carry a Spotify id. But every row belongs to one of three PSEUDO-playlists — `Favorite Songs` 4,078 · `Favorite Albums` 202 (199 after 3 exact duplicates) · `Favorite Artists` 163. **Zero real playlists.** The export captured the LIBRARY and none of the playlists the user made or saved. Not recoverable from this file — it needs a re-export with playlists enabled | ⏳ needs a new export from the user |
...
| # | Item | State |
|---|------|-------|
| 22 | ✅ **FIXED 2026-08-23 — `npm run lint` exits 0 on clean code and is usable as a gate.** The 6 errors were `Definition for rule 'react-hooks/exhaustive-deps' was not found`: 17 files carry 36 `eslint-disable-next-line` comments for a rule that was never registered, and ESLint errors on a directive disabling a rule it does not know. Registered `eslint-plugin-react-hooks`; both rules are WARNINGS (your call). 16 genuinely dead imports auto-removed. **`reportUnusedDisableDirectives` is now OFF** — see 22b | ✅ fixed |
| 22b | ⚠️ **A NEW `--fix` HAZARD, found by running it: it DELETES `eslint-disable` comments.** ESLint 9 defaults `reportUnusedDisableDirectives` to warn, and `--fix` strips the directives it deems unused, leaving stray whitespace. The first run stripped `react-hooks/exhaustive-deps` disables from **24 files** — comments that sit directly above deliberately-omitted deps and whose reasoning is in the folder CLAUDE.md. Reverted; the directive check is now `off` so `--fix` can only touch imports. This is the SECOND form of this hazard (2026-07-14: stripping JSX-only imports gutted the bundle) | ✅ guarded |
| 22c | ✅ **FIXED 2026-08-23 — `rules-of-hooks` is 0 across source.** All ten: `ActionConfig` (a `useMemo` inside `case "FIND"`), `ModuleContainer` (an early return above two hooks), `TextblockCard` (two hooks below the link return), and `Field` ×5 (four in the display-only section, one in the `date` branch). Each has a real trigger — changing a step's action type, the view-mode switcher, `convertRelink` writing `meta.link`, and the Fields tab editing `type`/input/display. Two regression suites, both A/B'd | ✅ done |
...
| 16 | ✅ **FIXED 2026-08-23 — the preview now follows what a doc DRAWS.** `PagePreviewApp` walked `occurrences[]` and `parentId` and stopped; a textmap embed is a THIRD reachability path, and **474 embeds across 233 hosts** are reachable only that way. Verified on prod against the exact cases: `Resume` **12 placeholders → 0** (11,630 → 14,550 chars), and the pre-codex `Eminem` page — the control — went from **50 chars** to **56,024** with 0 placeholders. Its whole body had been invisible in preview. 11 tests (cycle, dangling ref, nested node, compressed textmap) | ✅ done |
...
| C1 | ✅ **FIXED 2026-08-23 (`0214`) — the tasks goal is ONE number.** `Tasks Left.startValue` now DECLARES that it follows `Tasks Completed.targetValue` (`meta.deriveDisplayFrom`); the editor shows it read-only with *“follows Tasks Completed”* and the renderer resolves it. **It writes no value** — the stored number stays as the fallback and `resolveDisplayConfig` fails soft on a missing source, a missing value, a self-reference or a chain, so nothing can blank a goal tile. The pair agreed at 5, so applying it changed nothing on screen. Verified on prod (read-only + note, identity-checked, 0 page errors) and through the real resolver over live data: Completed target 5/12/3 → Left start 5/12/3 | ✅ fixed |
| C2 | **Field keys that are LIVE and have no control anywhere in the app** — measured against the 247 live fields, then grepped for a writer (readers do not count): `meta.multiSelect` on **45 fields** (four read sites in `Field.jsx` — whether a dropdown takes one pick or many is seed/migration-only), `meta.postfixOptions` on 19 (`helpers/fieldAffix.js`), `meta.min`/`meta.max` on 7/4 (the number clamp), and `siblingLinks` on all 247. `FieldDetail` renders fourteen controls and there is no raw-meta editor, so these are reachable only by writing a migration | ✅ fixed (5) |
| C3 | ✅ **FIXED 2026-08-23 (7)** — see the closed row above. *(original)* **`Operation.targetOccurrenceId` has NO editor at all** — 9 live ops carry it and it is load-bearing: the executor resolves `$activePeriodDates` from the op's OWN page through it (CLAUDE.md 2026-08-09 (8)). Every `targetOccurrenceId` match in the client is the unrelated `commitApplyTemplate` argument. So the one field that decides which page a date-driven op reads is invisible to the operations UI | ✅ fixed (7) |
| C4 | **Two field-meta keys are INERT** — `meta.increment` on **36 fields** and `meta.multiline` on 5 are read by nothing (`DrilldownTimePicker`'s `increment` is unrelated local state). The inert-token class this repo keeps rediscovering; either wire them or drop them. **Checked and NOT a gap:** `meta.workoutGoal` on 26 fields is `0170`'s own idempotency marker, correctly absent from the UI | ✅ fixed (5) |
...
| 2b | ✅ **DONE 2026-08-23 (morning, other account) — I reported this as open and it was not.** You asked *"what are those orphans and why do we keep having so many of them"* and got a measured answer: the **amplifier** is that placing a row CLONES its module (`cloneSubtree` mints one per node — 3,204 modules for 3,455 occurrences; 78 `Eat` modules for one concept), and the **leak** is that migrations bypass the runtime delete that sweeps the clone. You approved **a + b**, both shipped: an `orphan-module` integrity rule and `server/utils/migrationDelete.js`. The 135 were swept. Verified by running the sweeper today: *"No orphans, no dangling child refs, no module-less occurrences, no orphan modules."* | ✅ done |
...
| 3 | **MEASURED at last (attempt 5, 2026-08-23) — and the answer inverts the question.** The dark themes are NOT the problem. With the user's stored container colours neutralised (they are an overlay the theme does not control, and mixing them in is what defeated attempts 1-4), labels and headers read **12-16:1 on every skin**. Every sub-4.5 sample is the same thing: a **10px field-NAME caption inside a value pill**, deliberately dimmed — `rgba(34,51,102,0.65)` / `opacity 0.7` on moduli-light, `rgba(220,200,255,0.45)` on midnight. And **`moduli-light` is the WORST of the four** (16 of 18 under 4.5, min **1.76**) while midnight is 6 of 18, min 3.15. So this is one global choice about caption alpha, not a dark-theme defect. **Needs a call: raise the caption floor to ~4.5:1, or keep captions de-emphasised deliberately?** | ❓ measured, needs your call |
...
| 6b | ✅ **FIXED 2026-08-23 (`0208` + `0209`).** All 11 bills bind a visible `Completed` checkbox, and `Bills: Mark Paid` ticks the bill a `Pay Bill` row names. One way only — bills recur, so un-ticking a payment row does not mean unpaid. **Still reported, not fixed:** the amounts sum to 2220.97 against a frozen 2040.97 target (exactly Car Insurance) | ✅ done |
...
| 10 | ✅ **FIXED 2026-08-23 (9) — 3091ms → 2050ms blocked per date navigation (-34%).** `Schedule: Fill Day` was 766ms for ZERO effects: 3 layers × 49 slots × 1347 containers = 198,009 predicate evaluations to fill 13 slots. One added rule (`$tSlot.occurrences IS_NOT_EMPTY`) — provably inert, because the loop under the FIND reads the identical expression. Fill Day 766 → 127ms (-83%). Schedule verified still filling, byte-identical. **Remaining, filed not built:** hoisting the day's 49 slots into a var (~27× more) — expressible, but it depends on `$dayCol.occurrences` being populated in the overlay, and if it ever is not, Fill Day silently stops filling the schedule | ✅ fixed, bigger win filed |
| 10b | **The earlier diagnosis, kept for the record.** `resolveOptions` sharing shipped (772 rows × 7322 records → 45 answers; harness 2677ms → 0.3ms) and **the prod wall clock did not move** — a browser mounts 114 rows and 1 select, so the options resolver was never what a navigation waits on | ✅ correct, but not the fix |
| 13b | ✅ **CLOSED 2026-08-23 — keep them, your call.** *"the board is a catalog"*: Ingredients is what you CAN put in a meal, not this week's shopping list, so the twelve staples stay pickable. Not to be re-reported | ✅ closed |
...
| 14 | ✅ **DONE 2026-08-23 — and two of the three were already satisfied.** Measured on the live page first: *"the drag handle and the title should be on top of fields"* is ALREADY true (`handleTop === labelTop === 914`, `fieldTop 939`, `.instance-content` at `flex/column` — shipped 2026-08-11), and the containers ALREADY span their panel's full inner width. Only the tile width was outstanding, and it was arithmetic: 3 tiles at 168 + 2 gaps left **56px of every row empty**; 184 fills it and keeps three columns (200 would drop to two and waste more). `0215`/`0216` also boxed the four nested groups | ✅ done |
| 2c | **MEASURED 2026-08-23 — the premise IS violated, but for 924 modules, not 6,834.** User: *"shouldnt the module too be one, isnt that what the premise of the site is"* — yes, and here is where it holds and where it does not. Of 6,834 singleton modules, **3,470 are genuinely unique content** (a bookmark carrying its own URL in `fileRef`, an unlabelled textblock whose text lives on the occurrence) — those SHOULD be 1:1 and the premise does not apply. Of the 3,364 repeatable ones, **924 across 198 signatures are the same thing minted again and again**: `Exercise` ×26, `Daily Answer` ×25, `Tasks Completed` ×24, `Journal` ×24, `Notes` ×24 — one per day column. Collapsing those is **924 → 198, a saving of 726**. **The amplifier is the per-day template apply**, and the fix is a "reuse the source module when the clone would be identical" rule at the mint site. **The blocker is that there are TWO clone implementations** — `server/utils/cloneSubtree.js` (save/apply template) and the client executor's `APPLY_TEMPLATE` — so fixing one leaves the other minting. Needs an explicit go: it touches the paths that have damaged live data before | ❓ scoped, needs your go |
| 23 | ✅ **The stale-array clobber guard is BUILT** — flagged three times as "the durable fix" and I listed it as outstanding. `update_occurrence` now treats a provably-stale `occurrences[]` write as **ADDITIVE**: anything it drops that still exists is put back. Narrower than rejecting the write, and an intentional unlink through `CommitHelpers` is untouched. Paired with `utils/childRefGuard.js`, which stopped the dangling-ref guard asking the WRONG warm cache | ✅ done |
| 24 | ✅ **Browser clip extension** (other account) — right-click in the browser sends a selection → a row with the text in `Excerpt`, a page or link → a bookmark, an image → an image artifact, all through `/ingest`. Menus register on STARTUP as well as install (an MV3 worker is killed when idle, so `onInstalled`-only gives you a menu that works until the browser first reclaims memory). A 200 whose record failed is reported as a failure | ✅ shipped |
| 25 | ✅ **FIXED 2026-08-23 — and it was silently WRONG, not merely missing.** *"right click on the stuff inside the iframe … we should be able to add new occurances via right click"*. Chrome already fires the extension's menu inside frames, so the menu was there — but `info.pageUrl` and `tab.title` describe the HOST page, which for a bookmark opened in a panel is **Moduli itself**. Every clip taken from an embedded article was saved as *"Moduli"* pointing at `viafluere.com`, with no error. `clip.js` prefers `info.frameUrl` now and names a framed page by its own URL rather than borrowing the host tab's title; a link's and an image's own URL are left alone (already absolute). Identity is unchanged by where you clipped from, so a panel clip and a normal-tab clip of the same article are ONE row. A/B'd: 3 of 8 new tests fail against the old code | ✅ fixed |
| 25b | **The clip lands in the grid you CONFIGURED, not the grid you are looking at.** The remaining half of that ask. Settings hold one `gridId`; making a clip follow the open grid needs a content script on the Moduli origin reading `moduli-gridId` from `localStorage` and handing it to the worker. Small, but it adds a content script and an MV3 extension cannot be loaded in this environment, so it would ship unexercised | ❓ your call |
