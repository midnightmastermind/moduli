...
| # | Task | State |
|---|------|-------|
| G | **Tasks completed IN the Tasks container never reach `Completed`** — *"tasks that are being completed in the task container (not dragged to schedule though), is not moving to the completed section"* (08-22). `Tasks › Completed` is a materialized FEED (`0060`): `Completed IS true OR (Date DATE_BEFORE $today AND Time Slot IS_NOT_EMPTY)`, scoped to the Tasks page. `0180` already named the mechanism that breaks it — feed `scope` is an ancestor test resolved through `buildParentMap`, which keys child → ONE parent, **last writer wins** — and `96b0699a` fixed one case. A task never dragged to the schedule has only one parent, so this is a DIFFERENT failure from the one that fix addressed and needs its own measurement | 🐛 new, mechanism known |
| H | **Quick-add: FIELD-VALUE INPUTS in the menu, pre-filled and editable** — *"did you also add in the task to add quick apply values to added occurances / field values / we had that as a task as well"*, then clarified: *"its quick add new occurances with prefilled values, inside the quick add menu"* / *"its putting in inputs to put in prefilled values"* (08-22). So: picking a tile in the QuickAddMenu does not immediately mint a bare occurrence — it offers the occurrence's bound fields as **inputs**, already carrying the template's values, which you edit and then commit. The user was right that this was already a task: `CLAUDE_CHAT.md` carries it 18 times and `TASKS.md` carried it **zero**. The original (2026-08-06): *"i make one for movies and its already the picker, filled out, to the correct type occurance, and then in the ui form for specifics… we can have those filled out as part of these template. so then i can press add movie, and it just fills out the necessary new parts"*. **Two pieces already exist to build on:** `GET_USER_INPUT` renders a field-value modal, and the operations builder already has a create-step editor that adds and edits fields. Distinct from the dropdown prefill chain (`0042`), which shipped | 📋 was a task, never queued |
| I | **`Intake` → `Liquid Intake`, and the nutrition tiles get re-cut** — *"change Intake to Liquid Intake in trackers and remove meals, last meals, and meal count from there and add Meal Count with a goal of 3 to Meal Log. also remove Last Meal from the meal log as well"* (08-22). Concretely: `Intake` keeps `Daily Water` only; `Meal Log` loses `Last Meal` and gains `Meal Count` with target 3. **One thing to check first:** `Meal Count` is written by `Nutrition: Today's Micronutrients` onto the `Intake` occurrence, so moving the BINDING without moving the WRITE re-creates the exact bug `0184` just retired | 📋 new ask |
...
| A2 | **Open question it leaves:** on a rest day the tile renders a date and nothing else, which reads as broken even though it is correct. Worth a "Rest day" line rather than a blank tile — your call, not a bug | ❓ your call |
