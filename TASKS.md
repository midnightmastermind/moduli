...
| H | **Quick-add templates with pre-filled FIELD VALUES** — *"did you also add in the task to add quick apply values to added occurances / field values / we had that as a task as well"* (08-22). The user is right and it was missing: `CLAUDE_CHAT.md` carries it 18 times and `TASKS.md` **zero**. The original: *"templates in the sense of quick add item (i make one for movies and its already the picker, filled out, to the correct type occurance, and then in the ui form for specifics… we can have those filled out as part of these template. so then i can press add movie, and it just fills out the necessary new parts"*. Distinct from the dropdown prefill chain (`0042`), which shipped | 📋 was a task, never queued |
