...
| # | Task | State |
|---|------|-------|
| 1 | **`Place Weekday` loses one layer when two share a slot — a REAL defect, precisely reproduced.** `0177`'s whole point is merging layers into one day, and on a Friday `Cardio` (Run + Stretch) and `Meals` both target **7:00am**. Effect trace: `[0] CREATE Run into 7:00am · [1] CREATE Stretch into 7:00am · `[2] UPDATE 7:00am occurrences=1 includesCreated=0` — the second layer's parent-list write is a WHOLE-ARRAY overwrite from a stale snapshot and discards both. The last-writer-wins clobber this repo has paid for before. **Not fixed:** it is a shared executor write path (`ADD_CHILD` / APPLY_TEMPLATE's merge), and the emitters that patch `context.occurrencesById` and those that patch `$vars.$allOccurrences` do not see each other — which one the meal write reads is the next thing to establish | 🔴 reproduced, unfixed |
| 1b | **Land the refreshed fixture.** `mealTrackers`, `weekdayTemplates` and `weekdayTasks` are repaired and vintage-independent (stashed as `item-1 WIP`); `mergedTemplateLayers` is rewritten against real layered data and its one remaining failure IS item 1. Fixture reverted meanwhile so the shared checkout stays green | ⏳ blocked on 1 |
