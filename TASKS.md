...
| 25 | **"Add new occurrences via right click" INSIDE the iframe is NOT done** — *"is it possible to add a step at the end to add a windows menu shortcut so i can right click on the stuff inside the iframe and do stuff with it… we should be able to add new occurances via right click"* (08-23). The clip extension covers right-clicking in a NORMAL browser tab; it does not put your menu over the embedded iframe, and a cross-origin iframe cannot be reached from the page around it. Needs its own design — the extension can act inside the frame, but what it offers there is undecided | ❓ open ask, never filed |
