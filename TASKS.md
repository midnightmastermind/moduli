...
| # | Task | State |
|---|------|-------|
| J | **`Last Meal` is now bound by nothing while `Meal History` still writes it** — the leftover of `0190`. Harmless today (a write to a field no tile renders) but it is a value being computed for nobody. Either retire the write or give the field a home | 📋 reported, not fixed |
