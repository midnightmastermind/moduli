...
| 2c | **The orphan ROOT fix is still open, and it is the only one of the three that stops it recurring.** Option (c) from that same answer: stop cloning a module per placement — share the template and put per-row binding overrides on the occurrence. (a) makes accumulation visible and (b) gives migrations the tool, but the amplifier is untouched: every schedule rebuild still mints a day's worth of new modules. It touches clone, merge and template-apply, and `0117` had to bind `Set 4` on EVERY Exercise module precisely because of this design | ❓ your call, real architectural pass |
