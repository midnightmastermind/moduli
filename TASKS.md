...
| 22c | **10 real `rules-of-hooks` violations, reported and NOT diagnosed** — `OperationsBuilder:916`, `ModuleContainer:997,1001`, `TextblockCard:122,123`, `Field:1903,2056,2144,2148,2152`. All read *"React Hook is called conditionally"*. This is the class that has bitten this repo (BoundHeader had a `useMemo` after an early return — a changing hook COUNT). I could not find the early return the linter sees, so I will not claim they are false positives; they are warnings rather than silently switched off. **Do not raise the rule to error without fixing these**, or the gate goes red and stops being run | 🐛 reported, undiagnosed |
