...
describe("undo targets the top of the stack", () => {
  ...
  it("redo sends no transactionId either", () => {
    const socket = fakeSocket();
    const { result } = renderHook(() => useUndoRedo(socket, "g1"));

    act(() => { socket.fire("undo_state", { canUndo: false, canRedo: true, lastRedoableId: "tx-9" }); });
    act(() => { result.current.redo(); });

    const [, payload] = emitsOf(socket, "redo_transaction").at(-1);
    expect(payload).not.toHaveProperty("transactionId");
  });
