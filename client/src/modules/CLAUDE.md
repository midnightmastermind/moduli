...
- **ModulePanel.jsx** (rewritten 2026-08-21 — the dual-sidebar description below was true until then): ONE sidebar, on the RIGHT, behind a single `rootTreeOpen` state and a single show/hide button (`title="Files"`). It renders `<ManifestTree panelOccurrence={...} activePageView={...} />` — passing `panelOccurrence` is what makes the tree draw its **Pinned** section (this panel's open pages) above the full manifest. The LEFT rail and `localTreeOpen` are gone; there is no longer a Local-vs-Root toggle because there is no longer a choice to make. Drag-enter on the button still opens the sidebar so a drag can drop into the tree, and opening a page from the tree still collapses it.
