...
// ── LINK → BOOKMARK ────────────────────────────────────────────────────────
//
// Mints THE bookmark (`CommitHelpers.addBookmarkOccurrence`), the one the
// Browser tile, the viewer and Jonah make (user, 2026-09-22). This route used
// to mint a second shape — an instance record with Title/URL/Notes/Poster
// fields, from 2026-08-09 — which matched none of poms grid's bookmarks and
// refused on any grid without migration 0061.

/**
 * The grid's URL field, by NAME AND TYPE (two fields called "Due" is why name
 * alone is never enough here). Exported for the test.
 */
export function bookmarkFieldIds(fieldsById = {}) {
  const url = Object.values(fieldsById || {}).find(
...
function runLinkBookmark(ctx) {
  ...
  // THE bookmark — the one the Browser tile, the viewer and Jonah mint (user,
  // 2026-09-22). Its title and cover arrive from `link_preview` inside
  // `addBookmarkOccurrence`. The grid's URL field, when it has one, is bound
  // and filled so the bookmark stays filterable like the imported ones.
  const urlField = bookmarkFieldIds(fieldsById).url;
  const made = CommitHelpers.addBookmarkOccurrence({
    dispatch, socket, gridId, userId,
    containerOccurrence: destinationOccurrence,
    url, index: insertIndex,
    ...(urlField ? {
      fieldBindings: [{ fieldId: urlField, role: "input", order: 0 }],
      fields: { [urlField]: { value: url, flow: "in" } },
    } : null),
  });
  if (!made) { notifyIntake(ctx, { ok: false, error: "could not create the bookmark" }); return; }
  notifyIntake(ctx, { ok: true, message: "Bookmark added" });
}
